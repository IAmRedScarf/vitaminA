...
public class PzWKhm {
    ...
    class Solution {
        ...
        public int rob(int[] nums, int start, int end) {
            ...
            int[] dpRobCur = new int[nums.length];
            int[] dpNotRobCur = new int[nums.length];
            dpRobCur[start] = nums[start];
            for (int i = start + 1; i <= end; ++i) {
                dpRobCur[i] = dpNotRobCur[i - 1] + nums[i];
                dpNotRobCur[i] = Math.max(dpRobCur[i - 1], dpNotRobCur[i - 1]);
            }
            return Math.max(dpRobCur[end], dpNotRobCur[end]);
        }
    }
