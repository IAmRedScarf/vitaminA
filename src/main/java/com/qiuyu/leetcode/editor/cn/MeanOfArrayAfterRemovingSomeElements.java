...
public class MeanOfArrayAfterRemovingSomeElements {
    public static void main(String[] args) {
        ...
        solution.trimMean(new int[] {1,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,3});
    }
        ...
        public double trimMean(int[] arr) {
            ...
            for (int i = k; i < len; ++i) {
                ...
                if (arr[i] > minHeap.peek()) {
                    minSum = minSum - minHeap.poll() + arr[i];
                    minHeap.add(arr[i]);

                }
                if (arr[i] < maxHeap.peek()) {
                    maxSum = maxSum - maxHeap.poll() + arr[i];
                    maxHeap.add(arr[i]);
                }
            }
            return (allSum - minSum - maxSum) * 1.0 / (len - k - k);
