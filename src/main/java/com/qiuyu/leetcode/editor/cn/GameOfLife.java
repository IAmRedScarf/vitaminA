...
public class GameOfLife {
    ...
    class Solution {
        public void gameOfLife(int[][] board) {
            ...
            // 更新每个格子中的元素，用十位标识周围有多少个活细胞，个位不变
            for (int i = 0; i < rows; ++i) {
                for (int j = 0; j < cols; ++j) {
                    // 当一个格子中细胞存活时，更新其周围格子中的值
                    if (board[i][j] % 10 == 1) {
                        for (int k = 0; k < 3; ++k) {
                            for (int m = 0; m < 3; ++m) {
                                if (!(neighbours[k] == 0 && neighbours[m] == 0)) {
                                    int r = i + neighbours[k];
                                    int c = j + neighbours[m];
            ...
            for (int i = 0; i < rows; ++i) {
                for (int j = 0; j < cols; ++j) {
                    int aroundLiveNum = board[i][j] / 10;
                    int curStatus = board[i][j] % 10;

                    if (curStatus == 1 && (aroundLiveNum < 2 || aroundLiveNum > 3)) {
                        board[i][j] = 0;
                    } else if (curStatus == 0 && aroundLiveNum == 3) {
                        board[i][j] = 1;
                    } else {
                        board[i][j] = curStatus;
                    }
